...
variable {𝓐 Ω : Type*} {m𝓐 : MeasurableSpace 𝓐} {mΩ : MeasurableSpace Ω} [DecidableEq 𝓐]
  ...
  {h_inter : IsAlgEnvSeq O A R alg (Environment.bandit ν) P}
...
lemma condDistrib_reward'' [Countable 𝓐]
    (h : IsAlgEnvSeq O A R alg (Environment.bandit ν) P) (n : ℕ) :
    𝓛[fun ω ↦ R n ω.1 | fun ω ↦ A n ω.1; 𝔓] =ᵐ[(𝔓).map (fun ω ↦ A n ω.1)] ν := by
  have hA := h.measurable_action
  have hR := h.measurable_feedback
  have h_ra' : 𝓛[R n | A n; P] =ᵐ[P.map (A n)] ν := h.condDistrib_feedback_bandit n
...
lemma condIndepFun_reward_stepsUntil_action' [StandardBorelSpace Ω]
    (h : IsAlgEnvSeq O A R alg (Environment.bandit ν) P) (a : 𝓐) (m n : ℕ) :
  ...
  have h_indep : R n ⟂ᵢ[A n, hA n; P]
    ...
    IsAlgEnvSeq.condIndepFun_feedback_history_action_action_bandit h n
...
lemma condDistrib_rewardByCount_stepsUntil [StandardBorelSpace Ω] [Countable 𝓐]
    (h : IsAlgEnvSeq O A R alg (Environment.bandit ν) P) (a : 𝓐) (m : ℕ) (hm : m ≠ 0) :
...
lemma identDistrib_rewardByCount [StandardBorelSpace Ω] [Countable 𝓐]
    (h : IsAlgEnvSeq O A R alg (Environment.bandit ν) P) (a : 𝓐) (n m : ℕ)
    (hn : n ≠ 0) (hm : m ≠ 0) :
...
lemma identDistrib_rewardByCount_id [StandardBorelSpace Ω] [Countable 𝓐]
    (h : IsAlgEnvSeq O A R alg (Environment.bandit ν) P) (a : 𝓐) (n : ℕ) (hn : n ≠ 0) :
...
lemma identDistrib_rewardByCount_eval [StandardBorelSpace Ω] [Countable 𝓐]
    (h : IsAlgEnvSeq O A R alg (Environment.bandit ν) P) (a : 𝓐) (n m : ℕ) (hn : n ≠ 0) :
...
action at time `n`. -/
lemma indepFun_history_reward_cond (h : IsAlgEnvSeq O A R alg (Environment.bandit ν) P)
    (n : ℕ) (b : 𝓐) (k : ℕ) :
  ...
  exact h.indepFun_history_action_feedback_cond_bandit n
...
lemma hasLaw_reward_cond (h : IsAlgEnvSeq O A R alg (Environment.bandit ν) P) (t : ℕ) (b : 𝓐)
    (k : ℕ)
  ...
  exact h.hasLaw_feedback_cond_bandit t (measurableSet_snd_eq_and_pullCount'_eq t b k)
    (fun u hu ↦ hu.1) hP

lemma hasLaw_reward_cond_prod (h : IsAlgEnvSeq O A R alg (Environment.bandit ν) P) (t : ℕ) (b : 𝓐)
...
lemma indepFun_update_rewardByCountUntil_reward (h : IsAlgEnvSeq O A R alg (Environment.bandit ν) P)
    (t : ℕ) (b : 𝓐) (k : ℕ) :
    (fun ω ↦ Function.update (rewardByCountUntil A R t ω) (b, k) 0)
...
lemma identDistrib_rewardByCountUntil_add_one_cond
    (h : IsAlgEnvSeq O A R alg (Environment.bandit ν) P)
    (t : ℕ) (b : 𝓐) (k : ℕ) :
...
lemma identDistrib_rewardByCountUntil_add_one (h : IsAlgEnvSeq O A R alg (Environment.bandit ν) P)
    (t : ℕ) :
...
lemma hasLaw_rewardByCountUntil (h : IsAlgEnvSeq O A R alg (Environment.bandit ν) P) (t : ℕ) :
...
lemma hasLaw_rewardByCount_infinitePi (h : IsAlgEnvSeq O A R alg (Environment.bandit ν) P) :
...
lemma hasLaw_rewardByCount_add_one (h : IsAlgEnvSeq O A R alg (Environment.bandit ν) P)
    (a : 𝓐) (m : ℕ) :
...
all counts `m`. -/
lemma iIndepFun_rewardByCount_add_one (h : IsAlgEnvSeq O A R alg (Environment.bandit ν) P) :
...
and all counts `m`. -/
lemma iIndepFun_rewardByCount (h : IsAlgEnvSeq O A R alg (Environment.bandit ν) P) :
...
lemma iIndepFun_rewardByCount_add_one_action (h : IsAlgEnvSeq O A R alg (Environment.bandit ν) P)
    (a : 𝓐) :
...
lemma indepFun_rewardByCount (h : IsAlgEnvSeq O A R alg (Environment.bandit ν) P)
