...
`IdentAlg.stoppedHist A O X Y` (`Learning.stoppedHist`). If the algorithm never stops, the history
at the stopping time is by convention the empty history, and the output is then drawn from the
output rule at the empty history.
...
at `A.stoppingTime O X Y` (the empty history if `A` never stops). -/
...
conditional law `A.output` given the history at the stopping time (the empty history if `A`
never stops). -/
