...
def filtration (h : IsAlgEnvSeq A Y alg env P) :
  ...
  le' i := by
    rw [← measurable_iff_comap_le]
    exact Learning.measurable_history h.measurable_action h.measurable_feedback i

lemma adapted_history (h : IsAlgEnvSeq A Y alg env P) :
    Adapted h.filtration (history A Y) :=
...
lemma filtrationAction_eq_comap (h : IsAlgEnvSeq A Y alg env P) (n : ℕ) (hn : n ≠ 0) :
...
@[fun_prop]
lemma measurable_history_filtrationAction_add_one (h : IsAlgEnvSeq A Y alg env P) (n : ℕ) :
    Measurable[h.filtrationAction (n + 1)] (history A Y n) := by
  rw [h.filtrationAction_eq_comap (n + 1) (Nat.succ_ne_zero n)]
...
lemma filtration_le_filtrationAction_succ (h : IsAlgEnvSeq A Y alg env P) (n : ℕ) :
  ...
  measurable_iff_comap_le.mp (h.measurable_history_filtrationAction_add_one n)
