...
def IsAlgEnvSeq.filtration (h : IsAlgEnvSeq A Y alg env P) :
...
lemma IsAlgEnvSeq.adapted_history (h : IsAlgEnvSeq A Y alg env P) :
    Adapted h.filtration (history A Y) :=
  fun _ ↦ measurable_iff_comap_le.mpr le_rfl

lemma IsAlgEnvSeq.adapted_step (h : IsAlgEnvSeq A Y alg env P) :
...
lemma IsAlgEnvSeq.adapted_action (h : IsAlgEnvSeq A Y alg env P) :
...
lemma IsAlgEnvSeq.adapted_feedback (h : IsAlgEnvSeq A Y alg env P) :
...
def IsAlgEnvSeq.filtrationAction (h : IsAlgEnvSeq A Y alg env P) :
    Filtration ℕ mΩ where
  seq n := if n = 0 then MeasurableSpace.comap (A 0) inferInstance
    else IsAlgEnvSeq.filtration h (n - 1) ⊔ MeasurableSpace.comap (A n) inferInstance
  mono' n m hnm := by
    simp only
    by_cases hn : n = 0
    · by_cases hm : m = 0
      ...
      · simp only [hn, ↓reduceIte, hm]
        ...
        rw [← measurable_iff_comap_le]
        suffices Measurable[IsAlgEnvSeq.filtration h 0] (A 0) from
          this.mono ((IsAlgEnvSeq.filtration h).mono zero_le) le_rfl
    ...
    constructor
    · refine le_sup_of_le_left ?_
      exact (IsAlgEnvSeq.filtration h).mono hnm'
    · rcases eq_or_lt_of_le hnm with rfl | hlt
      ...
      suffices Measurable[h.filtration n] (A n) from
        this.mono ((IsAlgEnvSeq.filtration h).mono h_le) le_rfl
...
lemma IsAlgEnvSeq.filtrationAction_zero_eq_comap {h : IsAlgEnvSeq A Y alg env P} :
...
lemma IsAlgEnvSeq.filtrationAction_eq_comap
    {h : IsAlgEnvSeq A Y alg env P} (n : ℕ) (hn : n ≠ 0) :
