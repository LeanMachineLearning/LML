...
`Environment.bandit (κ.sectR e)`. Concretely, `bayesStationaryEnv Q κ : Environment 𝓔 𝓐 𝓨` announces
...
* `IsBayesAlgEnvSeq Q κ alg E A Y P`: states that the parameter `E : Ω → 𝓔` has law `Q` and that
  ...
  interacting with the stationary environment `Environment.bandit (κ.sectR (E ω))`.
...
* `ae_IsAlgEnvSeq h`: if `h : IsBayesAlgEnvSeq Q κ alg E A Y P`, for `Q`-almost every `e : 𝓔`,
  `IsAlgEnvSeq O' A' Y' alg (Environment.bandit (κ.sectR e)) (condDistrib (trajectory _ A Y) E P e)`
  for some sequence of actions `A' : ℕ → (ℕ → Round Unit 𝓐 𝓨) → 𝓐` and sequence of feedbacks
  ...
  measure that carries an `IsAlgEnvSeq` structure under the environment
  `Environment.bandit (κ.sectR e)` and the same algorithm. This allows transferring results from
  the `IsAlgEnvSeq` structure to the
...
(`Environment.bandit (κ.sectR (E ω))`).
...
lemma ae_IsAlgEnvSeq (h : IsBayesAlgEnvSeq Q κ alg E A Y P) :
    ∀ᵐ e ∂Q, IsAlgEnvSeq IT.obs IT.action IT.feedback alg (Environment.bandit (κ.sectR e))
      (condDistrib (trajectory (noObs Ω) A Y) E P e) := by
