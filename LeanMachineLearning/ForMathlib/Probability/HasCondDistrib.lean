...
omit [StandardBorelSpace Ω] [Nonempty Ω] in
/-- A conditional law given `X` is a conditional law given `X` under the restriction of `P` to an
event determined by `X`, that is, measurable for the σ-algebra generated by `X`. -/
lemma HasCondDistrib.restrict_of_measurableSet_comap [SFinite μ] [IsSFiniteKernel κ]
    (hX : Measurable X) (hY : Measurable Y)
    (h : HasCondDistrib Y X κ μ) {s : Set α} (hs : MeasurableSet[mβ.comap X] s) :
    HasCondDistrib Y X κ (μ.restrict s) := by
  obtain ⟨t, ht, rfl⟩ := MeasurableSpace.measurableSet_comap.1 hs
  exact h.restrict_preimage hX hY ht
